...
awk -F'[,!]' '$2=="BSSID" {print $1 "," $5 "," $3 "," $6}' _events.txt > events_s6.txt
awk -F'[,!]' '$4=="BSSID" {print $1 "," $3 "," $5 "," $6}' _events.txt >> events_s6.txt

sed -i 's/^/S6,/g' events_s6.txt

cat events_s6.txt | while read line ; do 
